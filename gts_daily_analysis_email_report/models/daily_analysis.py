...
class SaleOrder(models.Model):
    ...
    def scheduled_daily_analysis_report(self):
        ...
        inv_created = account_move_obj.search([('create_date', '>=', today_), ('create_date', '<=', today_),
                                               ('state', '!=', 'cancel'), ('invoice_payment_state', '!=', 'paid'),
                                               ('type', '=', 'out_invoice')])
        payment_created = payment_obj.search([('partner_type', '=', 'customer'), ('payment_date', '>=', today_),
                                              ('payment_date', '<=', today_)])
        bill_created = account_move_obj.search([('create_date', '>=', today_), ('create_date', '<=', today_),
                                                ('state', '!=', 'cancel'), ('invoice_payment_state', '!=', 'paid'),
                                                ('type', '=', 'in_invoice')])
        bill_payment_created = payment_obj.search([('partner_type', '=', 'supplier'), ('payment_date', '>=', today_),
                                                   ('payment_date', '<=', today_)])
        ...
        mrp_to_do = mrp_obj.search([('state', 'in', ('draft', 'confirmed', 'planned','progress', 'to_close')),('untaxed_amount','>',0)])
        ...
        for group_user in users_list:
            if group_user.has_group('gts_daily_analysis_email_report.group_daily_analysis_report_access'):
                ...
                for data in opp_created:
                    total_revenue += data.planned_revenue
                    deadline_date = ''
                    ...
                    """.format(partner=data.partner_id.name, opp_name=data.name, closing_date=deadline_date,
                               salesperson=data.user_id.name, probability=data.probability,
                               revenue=format_currency(data.planned_revenue, 'INR', locale='en_IN'))
                ...
                for data in crm_won:
                    ...
                    """.format(
                        partner=data.partner_id.name, opp_name=data.name,
                ...
                for data in crm_lost:
                    ...
                    """.format(partner=data.partner_id.name, opp_name=data.name,
                ...
                for data in quotation_prepared:
                    ...
                    margin += data.margin
                    total_sale_amount += data.amount_untaxed
                    total_tax_amount += data.amount_tax
                    total_po_value += data.po_value
                    ...
                    """.format(partner=data.partner_id.name,
                               po_amount=format_currency(data.po_value, 'INR', locale='en_IN'),
                               ...
                               salesperson=data.user_id.name,
                               status=dict(data._fields['state'].selection).get(data.state),
                               margin=format_currency(data.margin, 'INR', locale='en_IN'),
                               total_sale_price=format_currency(data.amount_untaxed, 'INR', locale='en_IN'),
                ...
                """
                total_amount, margin, avg_margin_percentage, total_sale_amount, total_cost_amount, total_tax_amount, total_po_value = 0, 0, 0, 0, 0, 0, 0
                for data in sale_orders:
                    ...
                    """.format(partner=data.partner_id.name,
                               total_amount=format_currency(data.amount_total, 'INR', locale='en_IN'),
                               po_value=format_currency(data.po_value, 'INR', locale='en_IN'),
                               salesperson=data.user_id.name,
                               name=data.name, margin_percentage="{:.2f}".format(data.margin_percentage),
                               margin=format_currency(data.margin, 'INR', locale='en_IN'),
                               total_sale_price=format_currency(data.amount_untaxed, 'INR', locale='en_IN'),
                ...
                for rec_po in purchase_rfq:
                    tax_totals += rec_po.amount_untaxed
                    tax_tot += rec_po.amount_tax
                    tax_total += rec_po.amount_total
                    body_purchase_rfq += """
                        <tr style="border: 1px solid">
                    ...
                    """.format(partner_id=rec_po.partner_id.name, user_id=rec_po.user_id.name, ref=rec_po.name or '',
                               amount_untaxed=format_currency(rec_po.amount_untaxed, 'INR', locale='en_IN'),
                               amount_tax=format_currency(rec_po.amount_tax, 'INR', locale='en_IN'),
                               amount_total=format_currency(rec_po.amount_total, 'INR', locale='en_IN'),
                               status=dict(rec_po._fields['state'].selection).get(rec_po.state))
                ...
                for rec in purchase_po:
                    tax_totals += rec.amount_untaxed
                    tax_tot += rec.amount_tax
                    tax_total += rec.amount_total
                    body_purchase_confirm += """
                        <tr style="border: 1px solid">
                            <td align="center" style="border: 1px solid">{partner_id}</td>
                            <td align="center" style="border: 1px solid">{ref}</td>
                    ...
                    """.format(partner_id=rec.partner_id.name, user_id=rec.user_id.name, ref=rec.name or '',
                               amount_untaxed=format_currency(rec.amount_untaxed, 'INR', locale='en_IN'),
                               amount_tax=format_currency(rec.amount_tax, 'INR', locale='en_IN'),
                               amount_total=format_currency(rec.amount_total, 'INR', locale='en_IN'))
                ...
                """
                total_untaxed_amount, total_tax_amount, final_total_amount, total_cost = 0, 0, 0, 0
                for data in inv_created:
                    total_untaxed_amount += data.amount_untaxed
                    total_tax_amount += data.amount_tax
                    final_total_amount += data.amount_total
                    total_cost += data.total_purchase_price
                    margin += data.margin
                    if total_untaxed_amount != 0.0:
                        avg_margin_percentage = (margin / total_untaxed_amount) * 100
                    body_inv_create += """
                        <tr style="border: 1px solid">
                            <td style="border: 1px solid">{partner}</td>
                    ...
                    """.format(partner=data.partner_id.name, origin=data.invoice_origin or '',
                ...
                for record in payment_created:
                    ...
                    """.format(partner=record.partner_id.name, payment_received=record.journal_id.name or '',
                               ref=record.communication or '', create_by=record.create_uid.name,
                               number=record.name or '',
                               amount=format_currency(record.amount, 'INR', locale='en_IN'),
                ...
                """.format(total_quantity=int(total_quantity),
                           untaxed_amount=format_currency(untaxed_amount, 'INR', locale='en_IN'))
